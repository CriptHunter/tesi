...
$$
g_R : D_1 \times D_2 \dots \times D_n \rightarrow rating
$$
In questo caso, il risultato è un cubo n-dimensionale al posto che una matrice bidimensionale. Le diverse dimensioni sono denotate come $D_1 \dots D_n$. Due di queste dimensioni saranno sempre utenti e oggetti, le altre $D_i$ dimensioni corrispondono alle features del contesto \cite{recsys-book}.
\begin{figure}
  \includegraphics[width=\linewidth, scale=0.60]{immagini/rating_cube.png}
  \caption{Esempio di un cubo multidimensionale per $User \times Item \times Location$}
  \label{fig:matrix_factorization}
\end{figure}
Il contesto può essere applicato nelle varie fasi del processo di raccomandazione. Si possono identificare tre categorie principali:
\begin{enumerate}
 ...
 \item \textit{Contextual post-filtering:} In questo paradigma, le informazioni contestuali sono inizialmente ignorate, e i ratings sono predetti utilizzando qualsiasi sistema di raccomandazione 2D traidizionale. Poi, l'insieme di raccomandazioni viene regolato utilizzando le informazioni di contesto.
 \item
\end{enumerate}
...
this recommendation paradigm (presented in Figure 7.4b), contextual information is initially ignored, and the ratings are predicted using any traditional 2D
