...
Il contesto fisico è composto da tutte quelle informazioni rilevanti che possono essere utilizzate per caratterizzare la situazione di un utente. Le feature del contesto fisico possono essere ricavate sfruttando i sensori presenti sugli smartphone personali di ogni utente, informazioni aggiuntive sul sistema operativo e altre informazioni esterne come data, ora e meteo. In particolare sono raccolte informazioni dalle seguenti fonti:

\paragraph{Posizione} Informazioni relative alla posizione geografica che includono latitudine, longitudine, precisione della posizione e direzione del movimento
\paragraph{Movimento utente} Il movimento dell'utente che include sia il movimento a piedi, come correre o camminare, sia il movimento su un mezzo di trasporto come l'automobile o la bicicletta.
\paragraph{Audio}  Informazioni relativi alla configurazione audio dello smartphone, che include la modalità audio (suono, vibrazione, silenzioso), il volume delle notifiche, e se l'altoparlante del telefono è acceso o spento.
\paragraph{Batteria} Informazioni relative alla batteria del telefono che includono il livello di carica e se la batteria si sta caricando. 
\paragraph{Display}  Stato dello schermo dello smartphone che indicano se è acceso, spento o si sta spegnendo perchè non è utilizzato, e orientamento dello schermo.
...
\paragraph{Celle di rete} Lista delle celle di rete mobile rilevante dal dispositivo. Per ogni cella si identifica il tipo di tecnologia (es. GSM o LTE), l'ID della cella, e la forza del segnale.
\paragraph{Wi-Fi} Lista di tutti gli access point Wi-Fi disponibili in prossimità, e se lo smartphone è connesso ad uno di essi.
\paragraph{Meteo}
