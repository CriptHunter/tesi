...
per valutare l'impatto del contesto nella produzione di raccomandazioni, confrontandolo con la sua variante context-aware ECAM NeuMF e con moveCARS. \`E infatti l'unico algoritmo di deep learning selezionato senza informazioni di contesto. Ho implementato il modello con la libreria Keras, seguendo il codice pubblicamente disponibile sul profilo GitHub\footnote{\url{https://github.com/hexiangnan/neural_collaborative_filtering}} di Xiangnan He, uno degli autori di \cite{NCF} in cui il modello è stato proposto.
\item \textit{ECAM NeuMF:} Delle tre varianti context-aware di NeuMF (ECAM, UCAM, HCAM), ho scelto ECAM NeuMF che utilizza un vettore di contesto esplicito senza ulteriori elaborazioni. Nonostante ECAM NeuMF sia la versione meno performante delle tre varianti \cite{context-aware-deep-learning}, è anche l'unica plausibile per essere implementata su dispositivi mobili. Questo modello è utile per un confronto diretto con moveCARS dato che i due modelli utilizzano lo stesso insieme di feature di contesto. Gli autori del modello non hanno rilasciato il codice sorgente o dettagli sull'implementazione. Per questo motivo ho implementato ECAM NeuMF in Keras, a partire da NeuMF.
\end{enumerate}
...
Per le proprietà elencate, ho scelto di usare l'AUC come metrica di paragone sia per il tuning degli iperparametri, sia per valutare le prestazioni dei modelli proposti nella \autoref{sec:comp-model} rispetto a moveCARS.

\subsection{AUC per ALS}
Per calcolare l'AUC di una rete neurale si divide il dataset $D$ in due sottoinsiemi $D_{train}$ e $D_{val}$. Il modello è addestrato su $D_{train}$ e valutato su $D_{val}$ calcolando l'AUC in base ai valori predetti dalla rete $y_{pred}$ e i valori reali $y_{test}$ nel dataset $D_{val}$. Per gli algoritmi di matrix factorization come ALS, la divisione in train e test della matrice originale avviene mascherando una percentuale dei feedback positivi dalla matrice originale. L'AUC è poi calcolata sulla lista prodotta delle $k$ migliori raccomandazioni, o sull'errore di ricostruzione della matrice prodotta da ALS rispetto alla matrice originale.
Per confrontare ALS con gli altri modelli ho seguito l'approccio suggerito in \cite{auc-als}, che punta a calcolare l'AUC solo per gli utenti che hanno avuto uno o più feedback mascherati, al posto che su tutta la matrice. L'algoritmo \ref{alg:auc-als} riporta lo pseudocodice della procedura. Gli input sono una matrice $D_{train}$ che corrisponde alla matrice originale $D$ con una percentuale di feedback positivi alterati in feedback negativi, $D_{test}$ che è una matrice i cui feedback positivi corrispondono ai feedback mascherati da $D_{train}$, e $D_{pred}$ che è la matrice ricostruita dal modello ALS dopo essere stato addestrato su $D_{train}$. L'output è l'AUC di ALS su $D$. L'algoritmo inizia creando una lista di tutti gli utenti che hanno almeno un feedback alterato (riga 1). Per ogni utente $u$ nella lista degli utenti alterati (riga 2), si trovano i vettori $u_{train}$, $u_{test}$ e $u_{pred}$ che corrispondono alla riga dell'utente $u$ nelle matrici $D_{train}$, $D_{test}$ e $D_{pred}$ (righe 3, 4, 5). Poi viene generato un vettore di indici $u_{idx}$ che denota le posizioni in $u_{train}$ che hanno feedback negativo (riga 6). Il motivo per cui si fa questo è che non vanno considerati nel calcolo dell'AUC i feedback positivi noti in fase di training che sono zero in $u_{test}$. Dei vettori $u_{test}$ e $u_{pred}$ vengono considerati solo gli elementi in posizione $u_{idx}$ (righe 7, 8). A questo punto viene calcolata con la funzione $auc\_score$, l'AUC per l'utente $u$ tra $u_{test}$ e $u_{pred}$ (riga 9). Il risultato $u_{auc}$ è sommato all'AUC globale (riga 10). Calcolata l'AUC per tutti gli utenti $u$, l'AUC globale viene ritornata (riga 12).
...
In \autoref{fig:result-frappe} è riportato un grafico con i risultati dei modelli sul dataset Frappe. Come per MDF l'algoritmo peggiore è ALS con un AUC pari a 0.7576. NeuMF ha un AUC migliore di ALS pari a 0.8143. Rimane comunque inferiore della sua controparte con il contesto ECAM NeuMF con un AUC pari a 0.8409. Nonostante Frappe sia un dataset con poche informazioni contestuali, il contesto aiuta comunque nel processo di raccomandazione, anche se ovviamente la differenza tra NeuMF e ECAM NeuMF è meno marcata che nel caso del dataset MDF. moveCARS ha ottenuto un AUC migliore di ALS e NeuMF, ma leggermente inferiore di ECAM NeuMF. Questo dimostra che anche con un contesto limitato, moveCARS riesce ad ottenere risultati vicini allo stato dell'arte.
...
\item \textit{Conversione del modello:} Il file salvato è dato in input al convertitore TF Lite che converte il modello TensorFlow in un modello TF Lite, e lo salva in un file con estensione tflite.
...
A questo punto si può utilizzare l'applicazione Android BenchmarkModel\footnote{\url{https://github.com/tensorflow/tensorflow/tree/master/tensorflow/lite/tools/benchmark}} per valutare i tempi di inizializzazione e inferenza dei modelli. Questa applicazione genera casualmente dei dati per un modello tflite e monitora il tempo che il modello impiega per inizializzare i propri parametri ed essere pronto per fare inferenza, e il tempo effettivo di inferenza sui dati generati casualmente. Ci sono alcuni parametri che possono essere configurati prima di eseguire il benchmark, tra cui:
\begin{itemize}
...
\item \texttt{num\_benchmark} \\ Il numero di benchmark eseguiti, il risultato finale è la media di tutti i benchmark. Ho scelto di eseguire 10 benchmark per ogni modello.
\item \texttt{num\_runs} \\ Il numero di esecuzioni in un benchmark. Aumentare questo valore riduce la varianza. Ho usato un numero di esecuzioni pari a 1000.
\item \texttt{use\_gpu} \\ Valore booleano che permette di selezionare se usare oppure no la GPU del dispositivo mobile. Per ragioni di stabilità ho impostalo il valore su falso.
\end{itemize}

\subsection{Risultati benchmark su Android}

\noindent Per valutare nel modo più accurato possibile i modelli, ho usato quattro diversi smartphone Android, su cui sono stati valutati i tempi di inizializzazione e inferenza dei modelli usando l'applicazione Android BenchmarkModel:
\begin{itemize}
\item \textit{Google Nexus 5:} \`E un telefono rilasciato sul mercato a fine 2013. Monta un processore Qualcomm Snapdragon 800 e ha 2GB di memoria RAM. Il sistema operativo è la versione base non modificata di Android.
\item \textit{Google Nexus 6:} \'E un telefono rilasciato sul mercato a fine 2014. Monta un processore Qualcomm Snapdragon 805 e ha 3GB di memoria RAM. Come per il Nexus 5, anche il Nexus 6 ha una versione stock di Android.
\item \textit{Samsung Galaxy S9 Plus:} \'E un telefono rilasciato sul mercato a inizio 2018. Monta un processore Samsung Exynos 9810 e ha 6GB di memoria RAM. Utilizza una versione Android modificata da samsung chiamata Samsung Experience. \`E l'unico tra i dispositivi che ho usato per il benchmark ad avere un processore diverso dai Qualcomm Snapdragon.
\item \textit{Xiaomi Mi9T:} \`E un telefono rilasciato sul mercato a inizio 2019. Monta un processore Qualcomm Snapdragon 730  e ha 6GB di memoria RAM. Utilizza una versione Android modificata da Xiaomi chiamata MIUI. \`E il dispositivo più recente tra quelli usati per il benchmark.
\end{itemize}
...
\noindent In \autoref{fig:init-time} è mostrato il tempo di inizializzazione dei modelli basati su deep learning sui quattro smartphone appena citati. Il grafico riporta il tempo di init in microsecondi, calcolato come la mediana tra i 10 benchmark. Ho usato la mediana al posto della media per eliminare gli outlier causati probabilmente dall'esecuzione di altri processi o applicazioni sul dispositivo durante la fase di benchmark. Tutti gli algoritmi di deep learning sono molto veloci nella fase di inizializzazione, con un tempo mediano che varia tra 418$\mu s$ e 2196,50$\mu s$. I tempi di inizializzazione più bassi sono ottenuti su Mi9T e S9 Plus che sono i dispositivi più recenti. MoveCARS è leggermente più veloce di NeuMF e ECAM NeuMF su tutti i dispositivi Android.  

In \autoref{fig:inference-time} è mostrato il tempo di inferenza su dispositivi mobili. Anche in questo caso il grafico riporta i tempi di inferenza in microsecondi calcolati come la mediana tra 10 benchmark. I tempi di inferenza sono molto più bassi di quelli di inizializzazione con il tempo mediano per una singola inferenza che varia tra 13.50$\mu s$ e 57.74$\mu s$. In questo caso il dispositivo più veloce è stato il Mi9T, mentre gli altri smartphone si attesta su risultati simili. Per quanto riguarda i modelli di deep learning, anche in fase di inferenza moveCARS è più veloce, seppure si parla di differenze di pochi microsecondi. In conclusione, dati i test effettuati su smartphone ormai datati e più recenti si può dire che tutti i modelli possono eseguire in real time.
