...
A dataset with many features naturally leads to high-dimensional space and consequently leads to sparsity with maybe only a small number of features per object. This is known as curse of dimensionality [21].


I context-aware recommender system usano principalmente un set predefinito di informazioni contestuali esplicite. Il contesto specifico descrive la situazioni in cui le informazioni sono state raccolte come il meteo (soleggiato...),  o il tempo (weekday,weekend, ...). Il vantaggio principale del contesto esplicito è che si può integrare facilmente nei recommender system esistenti data la bassa dimensionalità. Tuttavia, i CARS espliciti hanno le seguenti limitazioni: 1. la selezione del contesto specifico è un task che richiede tempo essendo fatto a mano dagli esperti, 2. il contesto selezionato potrebbe non rappresentare il set di features contestuali più efficace per il recsys in questione. 3. privacy...
La limitazione sul numero di feature contestuali è un problema quando è possibile ricavare feature di contesto da numerosi sensori dei telefoni come accelerometro, campo magnetico, GPS, e luce e molto altro. L'accelerometro può essere usato per inferire l'attiivtò dell'utente (camminare, seduto), mentre altre feature come il GPS per la posizione
