...
\begin{itemize}
 \item \textit{Problema del cold-start:} Si riferisce alla situazione in cui il sistema di raccomandazione non ha abbastanza informazioni su un utente od un oggetto per poter fare previsioni rilevanti. Il profilo del nuovo utente o del nuovo oggetto sarà vuoto perchè non ha valutato nessun oggetto; quindi, i suoi gusti non saranno conosciuti al sistema \cite{recsys-principle-methods-evaluation}.   
...
\textbf{Vantaggi:}
\begin{itemize}
 \item \textit{Consigliare nuovi oggetti:} Questi modelli hanno la capacità di consigliare nuovi oggetti anche se non ci sono valutazioni fornite dagli utenti. Non soffrono quindi del problema del cold-start dei collaborative filtering \cite{recsys-principle-methods-evaluation}.
...
\begin{enumerate}
 \item \textit{Ensemble design:} Con questo metodo i risultati degli algoritmi base sono combinati in un output singolo più robusto. Il principio base è molto simile ai metodi di ensemble usati in molte applicazioni di data mining come clustering, classificazione e analisi degli outlier.  La combinazione può essere fatta in vari modi, ad esempio calcolando la media pesata delle varie predizioni. La caratteristica comune è che questi sistemi usano sistemi di raccomandazione già esistenti per produrre uno score/ranking unico.
